...
import type { Exercise } from '../../../interfaces/catalog';
import type { CreateWorkoutRequest, WorkoutSummary } from '../../../interfaces/workouts';
import {
  createWorkout,
  deleteWorkout,
  getWorkout,
  ...
  updateWorkout,
} from '../../../services/api/workouts/workoutsApi';
...
// EXCEPCIÓN DOCUMENTADA (FR-012): Los campos de estado difieren del contrato constitucional
// por razones de granularidad. Equivalencias semánticas:
//   items         ↔ list      (colección paginada de workouts)
//   formDraft     ↔ form      (estado completo del formulario de creación/edición)
//   isLoading*    ↔ loading   (desglosado por operación: isLoadingHistory, isLoadingWorkout, isDeleting, isSubmitting)
//   *Error        ↔ error     (desglosado por operación: historyError, createError, editError, deleteError)
//   isCreate/Edit/DeleteDialogOpen ↔ popUpCode (control de diálogos, sin código de popup único)
// La granularidad mejora la type-safety en RTK y evita re-renders innecesarios.
type WorkoutsState = {
  from: string;
  to: string;
