...
export function getCurrentDayExerciseTrainingLog(routineId: string, sessionId: string, exerciseId: string) {
  const params = new URLSearchParams({ routineId, sessionId, exerciseId });
  return apiFetch<ExerciseTrainingLog | null>(`/api/exercise-training-logs/current-day?${params.toString()}`);
}
