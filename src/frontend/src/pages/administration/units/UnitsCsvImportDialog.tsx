...
import { IUnit } from '../../../interfaces/admin/units/IUnit';
import { useAppDispatch, useAppSelector } from '../../../redux/hooks';
import { fetchUnits, importUnitsCsv, setUnitsPopUpCode } from '../../../redux/actions/units/unitsActions';
import { selectUnitState } from '../../../redux/states/units/unitsState';
import { PopUpCode } from '../../../enums/popUp/popUp';

function parseCsvRows(raw: string): IUnit[] {
  const lines = raw
    .split(/\r?\n/)
    .map((line) => line.trim())
...
export function UnitsCsvImportDialog() {
  const { t } = useTranslation();
  const dispatch = useAppDispatch();
  const { loading, error, popUpCode } = useAppSelector(selectUnitState);
  ...
  const handleImport = async () => {
    ...
    const result = await dispatch(importUnitsCsv(csvRaw));
    if (importUnitsCsv.fulfilled.match(result)) {
