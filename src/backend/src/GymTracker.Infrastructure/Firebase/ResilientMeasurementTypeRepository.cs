...
public sealed class ResilientMeasurementTypeRepository : IMeasurementTypeRepository
{
    private static int _fallbackMode;

    private readonly MeasurementTypeRepository _primary;
    private readonly InMemoryMeasurementTypeRepository _fallback;
    private readonly ILogger<ResilientMeasurementTypeRepository> _logger;

    public ResilientMeasurementTypeRepository(
        MeasurementTypeRepository primary,
        InMemoryMeasurementTypeRepository fallback,
        ILogger<ResilientMeasurementTypeRepository> logger)
    {
        _primary = primary;
        _fallback = fallback;
        _logger = logger;
    ...
    {
        return ExecuteWithFallback(
            () => _primary.ListAssignableAsync(cancellationToken),
            () => _fallback.ListAssignableAsync(cancellationToken));
    }

    public Task<MeasurementTypeResponse?> GetByIdAsync(string id, CancellationToken cancellationToken = default)
    {
        return ExecuteWithFallback(
            () => _primary.GetByIdAsync(id, cancellationToken),
            () => _fallback.GetByIdAsync(id, cancellationToken));
    }
    ...
    {
        return ExecuteWithFallback(
            () => _primary.CreateAsync(request, cancellationToken),
            () => _fallback.CreateAsync(request, cancellationToken));
    }
    ...
    {
        return ExecuteWithFallback(
            () => _primary.UpdateAsync(id, request, cancellationToken),
            () => _fallback.UpdateAsync(id, request, cancellationToken));
    }

    public Task<bool> DeleteAsync(string id, CancellationToken cancellationToken = default)
    {
        return ExecuteWithFallback(
            () => _primary.DeleteAsync(id, cancellationToken),
            () => _fallback.DeleteAsync(id, cancellationToken));
    }

    public Task<MeasurementTypeResponse?> ReactivateAsync(string id, CancellationToken cancellationToken = default)
    {
        return ExecuteWithFallback(
            () => _primary.ReactivateAsync(id, cancellationToken),
            () => _fallback.ReactivateAsync(id, cancellationToken));
    }
    ...
    private async Task<T> ExecuteWithFallback<T>(Func<Task<T>> primaryCall, Func<Task<T>> fallbackCall)
    {
        ...
        catch (RpcException exception) when (exception.StatusCode == StatusCode.ResourceExhausted)
        {
            ...
            _logger.LogWarning(
                exception,
                "Firestore quota exceeded for measurement types. Switching to in-memory fallback until application restart.");
